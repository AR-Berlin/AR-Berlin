...
AR-Berlin = Token:match("(%d+)")
redis:set(AR-Berlin..":VERSION",2.9)
redis:set(AR-Berlin..":SUDO_ID:",SUDO_USER)
redis:set(AR-Berlin..":DataCenter:",'German')
redis:set(AR-Berlin..":UserNameBot:",BOT_User)
redis:set(AR-Berlin..":NameBot:",BOT_NAME)
redis:hset(AR-Berlin..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3AR-Berlin_INSTALL","Yes")
info = {}
...
─────▀██▀─────
┊AR-Berlin ‿ @EEFFI
تم تطوير وبرمجة السورس من قبل ارماندو
┊@EEFFI  ‿ @EEFFI 
-------------------------------------------------------------------
]]..'\027[0;32m')
create_config() 
...
AR-Berlin = Token:match("(%d+)")
our_id = tonumber(AR-Berlin)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(AR-Berlin..":UserNameBot:")
SUDO_ID = tonumber(redis:get(AR-Berlin..":SUDO_ID:"))
SUDO_USER = redis:hgetall(AR-Berlin..'username:'..SUDO_ID).username
version = redis:get(AR-Berlin..":VERSION")
DataCenter = redis:get(AR-Berlin..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..AR-Berlin..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(AR-Berlin..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(AR-Berlin..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(AR-Berlin..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(AR-Berlin..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور ‍•'
msg.TheRank = 'مطور البوت ‍•'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(AR-Berlin..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي '
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(AR-Berlin..':F96BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء '
msg.TheRank = 'المنشىء '
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(AR-Berlin..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير •' 
msg.TheRank = 'مدير البوت •' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(AR-Berlin..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن •'
msg.TheRank = 'ادمن في البوت •'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(AR-Berlin..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(AR-Berlin..'lock_bots'..msg.chat_id_)
RG0BOT = false
...
if msg.GroupActive and RG0BOT and redis:get(AR-Berlin..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(AR-Berlin..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(AR-Berlin..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(AR-Berlin..':WELCOME_BOT'),[[‍♂╿ مـرحبآ آنآ بوت آسـمـي ]]..redis:get(AR-Berlin..':NameBot:')..[[ ⚜
...
if msg.GroupActive and redis:get(AR-Berlin..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(AR-Berlin..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(AR-Berlin..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(AR-Berlin..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.AR-Berlin then 
if msg.text and ScriptFile.iAR-Berlin then
for k, AR-Berlin in pairs(ScriptFile.AR-Berlin) do
local SearchText = msg.text:match(AR-Berlin)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.iAR-Berlin(msg,{msg.text:match(AR-Berlin)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",AR-Berlin.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dAR-Berlin then
if ScriptFile.dAR-Berlin(msg) == false then
return false
end
...
if Plug.AR-Berlin then 
if msg.text and Plug.iAR-Berlin then
for k, AR-Berlin in pairs(Plug.AR-Berlin) do
local SearchText = msg.text:match(AR-Berlin)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(AR-Berlin..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(AR-Berlin..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.iAR-Berlin(msg,{msg.text:match(AR-Berlin)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",AR-Berlin.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
if Plug.dAR-Berlin then
Plug.dAR-Berlin(msg)
print("\27[1;35m¦Msg_IN_Process : \27[1;32"..name.."\27[0m")
end
else
print("The File "..name.." Not Runing in The  AR-Berlin")
end 

end
else
print("The File Script.lua Not Runing in The  AR-Berlin")
...
function tdcli_update_callback(data)
	...
	redis:srem(AR-Berlin..'users',msg.sender_user_id_)
	end
	...
	if UpdateAR-BerlinStart then
	UpdateAR-BerlinStart = false
	EditMsg(data.message_.chat_id_,data.message_.id_,'10% - |█          |')
	EditMsg(data.message_.chat_id_,data.message_.id_,'20% - |███         |')
	download_file('https://raw.githubusercontent.com/AR-Berlin/AR-Berlin/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/AR-Berlin/AR-Berlin/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/AR-Berlin/AR-Berlin/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/AR-Berlin/AR-Berlin/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/AR-Berlin/AR-Berlin/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/AR-Berlin/AR-Berlin/master/plugins/games.lua','./plugins/games.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'100% - |█████████████|\n\n🔝*¦* السورس الى اصدار \n📟*¦* تم اعاده تشغيل السورس بنجاح')
	dofile("./inc/Run.lua")
	print("Update AR-Berlin And Reload ~ ./inc/Run.lua")
	end
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	if msg.text== 'Update AR-Berlin' and msg.sender_user_id_ == SUDO_ID then
	download_file('https://raw.githubusercontent.com/AR-Berlin/AR-Berlin/master/inc/Run.lua','./inc/Run.lua')
	download_file('https://raw.githubusercontent.com/AR-Berlin/AR-Berlin/master/inc/Script.lua','./inc/Script.lua')
	download_file('https://raw.githubusercontent.com/AR-Berlin/AR-Berlin/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/AR-Berlin/AR-Berlin/master/inc/locks.lua','./inc/locks.lua')
	...
	mmd = redis:get(AR-Berlin..":SUDO_ID:")
redis:hset(AR-Berlin..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(AR-Berlin..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(AR-Berlin..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(AR-Berlin..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(AR-Berlin..':F96BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(AR-Berlin..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(AR-Berlin..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(AR-Berlin..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if RG0BOT and redis:get(AR-Berlin..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(AR-Berlin..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(AR-Berlin..'group:add'..data.chat_.id_) then
	redis:set(AR-Berlin..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(AR-Berlin..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(AR-Berlin..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(AR-Berlin..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	AR-Berlin..'group:name'..k,IDS.Title,
	AR-Berlin..'num_msg_AR-Berlin'..k,5,
	AR-Berlin..'group:add'..k,true,
	AR-Berlin..'lock_link'..k,true,
	AR-Berlin..'lock_spam'..k,true,
	AR-Berlin..'lock_webpage'..k,true,
	AR-Berlin..'lock_markdown'..k,true,
	AR-Berlin..'lock_flood'..k,true,
	AR-Berlin..'lock_bots'..k,true,
	AR-Berlin..'mute_forward'..k,true,
	AR-Berlin..'mute_contact'..k,true,
	AR-Berlin..'mute_document'..k,true,
	AR-Berlin..'mute_inline'..k,true,
	AR-Berlin..'lock_username'..k,true,
	AR-Berlin..'replay'..k,true
	)
	redis:sadd(AR-Berlin..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(AR-Berlin..'username:'..ID,'username',user)
	redis:sadd(AR-Berlin..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(AR-Berlin..'username:'..ID,'username',user)
	redis:sadd(AR-Berlin..':F96BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(AR-Berlin..'username:'..ID,'username',user)
	redis:sadd(AR-Berlin..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'📦*¦* تم رفع آلنسـخهہ‏‏ آلآحتيآطـيهہ\n⚖️*¦* حآليآ عدد مـجمـوعآتگ هہ‏‏يهہ‏‏ *'..redis:scard(AR-Berlin..'group:ids')..'* 🌿\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(AR-Berlin..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(AR-Berlin..'username:'..data.user_.id_).username
	...
	redis:hset(AR-Berlin..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
